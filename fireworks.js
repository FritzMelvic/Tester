...
const fastShellBlacklist = ["Falling Leaves", "Floral", "Valentine Heart", "Valentine Image", "Willow"];
...
const shellTypes = {
  ...
  Strobe: strobeShell,
  Willow: willowShell,
  "Valentine Image": valentineImageShell,
  "Valentine Heart": valentineHeartShell,
};
...
function valentineImageShell(size) {
  ...
  shell.burst = function (x, y) {
    ...
    img.onload = () => {
      ...
      const data = tempCtx.getImageData(0, 0, resolution, resolution).data;
      const scale = this.spreadSize / resolution;
      ...
      for (let py = 0; py < resolution; py++) {
        for (let px = 0; px < resolution; px++) {
          ...
          if (alpha > 120) {
            ...
            const vx = (px - resolution / 2) * scale * 0.05;
            const vy = (py - resolution / 2) * scale * 0.05;
      ...
      BurstFlash.add(x, y, shell.spreadSize);
...
function seqValentineShow() {
  const shell1 = new Shell(valentineHeartShell(shellSizeSelector()));
  shell1.launch(0.5, 0.6);
...
function valentineHeartShell(size) {
  const shell = new Shell({
    spreadSize: size * 100,
    starLife: 2500,
    starCount: 0, // MATIKAN BURST DEFAULT
    color: COLOR.Red,
    glitter: "medium",
    glitterColor: COLOR.White
  });
  ...
  shell.burst = function (x, y) {
    const count = 300;
    const scale = this.spreadSize / 45;
  ...
  for (let i = 0; i < count; i++) {
    ...
    const vx = rawX * scale * 0.05;
    const vy = -rawY * scale * 0.05;

    // 🎨 RANDOM WARNA LEBIH HIDUP
    const baseColor =
      Math.random() < 0.5 ? COLOR.Red : COLOR.Gold;
    ...
    // 💫 FADE WARNA SEBELUM MATI
    star.secondColor = COLOR.White;
    star.transitionTime = this.starLife * 0.6;
}
    ...
    for (let i = 0; i < count; i++) {
      const t = Math.PI * 2 * (i / count);
         ...
         // INI KUNCINYA:
         // Canvas Y positif ke bawah, jadi kita balik Y
          const vx = rawX * scale * 0.05;
          const vy = -rawY * scale * 0.05;
    ...
    BurstFlash.add(x, y, this.spreadSize);
    soundManager.playSound("burst");
  };
...
function startSequence() {
  ...
  if (finaleSelector()) {
    if (currentFinaleCount < finaleCount) {
      ...
      // ⏳ DELAY LAUNCH HEART 1500ms
      setTimeout(() => {
      const shell = valentineHeartShell(shellSizeSelector());
